...
typedef int (*swig_wrapper_func)(ClientData,
                                 Tcl_Interp*,
                                 int,
                                 Tcl_Obj* CONST[]);
                                        ...
                                        Tcl_Obj*    obj)
{
  int newobj;
  Tcl_ObjSetVar2(
      interp, Tcl_NewStringObj(name, -1), NULL, obj, TCL_GLOBAL_ONLY);
  Tcl_SetHashValue(Tcl_CreateHashEntry(&swigconstTable, name, &newobj),
                   (ClientData) obj);
}
...
#define SWIG_GetModule(clientdata) \
  SWIG_Tcl_GetModule((Tcl_Interp*) (clientdata))
                                          ...
                                          int             flags)
{
  return SWIG_Tcl_ConvertPtrFromString(
      interp, Tcl_GetStringFromObj(oc, NULL), ptr, ty, flags);
}
...
SWIGRUNTIME int SWIG_Tcl_ConvertPacked(Tcl_Interp*     SWIGUNUSEDPARM(interp),
                                       Tcl_Obj*        obj,
                                       void*           ptr,
                                       int             sz,
                                       swig_type_info* ty)
...
SWIGRUNTIME swig_module_info* SWIG_Tcl_GetModule(Tcl_Interp* interp)
{
  ...
  data
      = Tcl_GetVar(interp,
                   (char*) "swig_runtime_data_type_pointer" SWIG_RUNTIME_VERSION
  ...
  while (1) {
    ...
    if (bi != -1) {
      if (!cls->bases[bi] && cls->base_names[bi]) {
        ...
        swig_type_info* info = SWIG_TypeQueryModule(
            cls->module, cls->module, cls->base_names[bi]);
        if (info)
          cls->bases[bi] = (swig_class*) info->clientdata;
      }
      cls = cls->bases[bi];
      if (cls) {
  ...
  while (cls) {
    ...
    while (meth && meth->name) {
      ...
      while (where) {
        ...
        if (where) {
          if (where[-1] == ' '
              && (where[meth_len] == ' ' || where[meth_len] == 0)) {
            break;
          } else {
            where++;
          }
    ...
    if (!Tcl_GetCommandInfo(interp, name, &ci) || (flags)) {
      ...
      newinst->cmdtok
          = Tcl_CreateObjCommand(interp,
                                 Tcl_GetStringFromObj(robj, NULL),
                                 (swig_wrapper_func) SWIG_MethodCommand,
                                 (ClientData) newinst,
                                 (swig_delete_func) SWIG_ObjectDelete);
      if (flags) {
                                           ...
                                           Tcl_Obj* CONST objv[])
{
  ...
  if (!classptr) {
    Tcl_SetResult(
        interp,
        (char*) "swig: internal runtime error. No class object defined.",
        TCL_STATIC);
    return TCL_ERROR;
  }
  ...
  if (SWIG_Tcl_ConvertPtr(
          interp, newObj, (void**) &thisvalue, *(classptr->type), 0)
      != SWIG_OK) {
  ...
  newinst->cmdtok = Tcl_CreateObjCommand(interp,
                                         name,
                                         (swig_wrapper) SWIG_MethodCommand,
                                         (ClientData) newinst,
                                         (swig_delete_func) SWIG_ObjectDelete);
  return TCL_OK;
}
  ...
  for (c = fmt; (*c && (*c != ':') && (*c != ';')); c++, argno++) {
    ...
    if (vptr) {
      ...
      switch (*c) {
        ...
        case 'b':
        case 'B':
          if (Tcl_GetLongFromObj(interp, obj, &tempi) != TCL_OK)
            goto argerror;
          if ((*c == 'i') || (*c == 'I'))
            *((int*) vptr) = (int) tempi;
          else if ((*c == 'l') || (*c == 'L'))
            ...
            *((unsigned char*) vptr) = (unsigned char) tempi;
          break;
        case 'f':
        case 'F':
        case 'd':
        case 'D':
          if (Tcl_GetDoubleFromObj(interp, obj, &tempd) != TCL_OK)
            goto argerror;
          if ((*c == 'f') || (*c == 'F'))
        ...
        case 'P':
          ...
          if (SWIG_Tcl_ConvertPtr(interp, obj, (void**) vptr, ty, 0) != SWIG_OK)
            goto argerror;
          break;
        case 'o':
        case 'O':
...
SWIGINTERNINLINE Tcl_Obj* SWIG_FromCharPtrAndSize(const char* carray,
                                                  size_t      size)
{
  return (size < INT_MAX) ? Tcl_NewStringObj(carray, static_cast<int>(size))
                          : NULL;
}
...
extern "C" {
#endif
SWIGINTERN int _wrap_new_OpenIRA(ClientData clientData SWIGUNUSED,
                                 Tcl_Interp*           interp,
                                 int                   objc,
                                 Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp, objc, objv, ":new_OpenIRA ") == TCL_ERROR)
    SWIG_fail;
  result = (OpenIRA*) new OpenIRA();
  Tcl_SetObjResult(
      interp,
      SWIG_NewInstanceObj(SWIG_as_voidptr(result), SWIGTYPE_p_OpenIRA, 0));
  return TCL_OK;
fail:
  return TCL_ERROR;
}

SWIGINTERN int _wrap_delete_OpenIRA(ClientData clientData SWIGUNUSED,
                                    Tcl_Interp*           interp,
                                    int                   objc,
                                    Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp, objc, objv, "o:delete_OpenIRA self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(
      objv[1], &argp1, SWIGTYPE_p_OpenIRA, SWIG_POINTER_DISOWN | 0);
  if (!SWIG_IsOK(res1)) {
...
SWIGINTERN int _wrap_OpenIRA_db_set(ClientData clientData SWIGUNUSED,
                                    Tcl_Interp*           interp,
                                    int                   objc,
                                    Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_db_set self db ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  ...
  res2 = SWIG_ConvertPtr(
      objv[2], &argp2, SWIGTYPE_p_odb__dbDatabase, SWIG_POINTER_DISOWN | 0);
  if (!SWIG_IsOK(res2)) {
...
SWIGINTERN int _wrap_OpenIRA_db_get(ClientData clientData SWIGUNUSED,
                                    Tcl_Interp*           interp,
                                    int                   objc,
                                    Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp, objc, objv, "o:OpenIRA_db_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
  ...
  Tcl_SetObjResult(interp,
                   SWIG_NewInstanceObj(
                       SWIG_as_voidptr(result), SWIGTYPE_p_odb__dbDatabase, 0));
  return TCL_OK;
fail:
  return TCL_ERROR;
}

SWIGINTERN int _wrap_OpenIRA_verilog_stor_set(ClientData clientData SWIGUNUSED,
                                              Tcl_Interp*           interp,
                                              int                   objc,
                                              Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_verilog_stor_set self verilog_stor ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_verilog_stor_get(ClientData clientData SWIGUNUSED,
                                              Tcl_Interp*           interp,
                                              int                   objc,
                                              Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_verilog_stor_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
  ...
  Tcl_SetObjResult(interp,
                   SWIG_From_std_string(static_cast<std::string>(*result)));
  return TCL_OK;
fail:
  return TCL_ERROR;
}

SWIGINTERN int _wrap_OpenIRA_lib_stor_set(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_lib_stor_set self lib_stor ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  ...
  arg1 = reinterpret_cast<OpenIRA*>(argp1);
  res2 = SWIG_ConvertPtr(
      objv[2], &argp2, SWIGTYPE_p_std__vectorT_std__string_t, 0 | 0);
  if (!SWIG_IsOK(res2)) {
    SWIG_exception_fail(SWIG_ArgError(res2),
                        "in method '"
                        "OpenIRA_lib_stor_set"
...
SWIGINTERN int _wrap_OpenIRA_lib_stor_get(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_lib_stor_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
  ...
  Tcl_SetObjResult(
      interp,
      SWIG_NewInstanceObj(
          SWIG_as_voidptr(result), SWIGTYPE_p_std__vectorT_std__string_t, 0));
  return TCL_OK;
fail:
  return TCL_ERROR;
}

SWIGINTERN int _wrap_OpenIRA_sdc_file_set(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_sdc_file_set self sdc_file ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_sdc_file_get(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_sdc_file_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
...
SWIGINTERN int _wrap_OpenIRA_top_cell_name_set(ClientData clientData SWIGUNUSED,
                                               Tcl_Interp*           interp,
                                               int                   objc,
                                               Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_top_cell_name_set self top_cell_name ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_top_cell_name_get(ClientData clientData SWIGUNUSED,
                                               Tcl_Interp*           interp,
                                               int                   objc,
                                               Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_top_cell_name_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
...
SWIGINTERN int _wrap_OpenIRA_vsrc_loc_set(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_vsrc_loc_set self vsrc_loc ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_vsrc_loc_get(ClientData clientData SWIGUNUSED,
                                          Tcl_Interp*           interp,
                                          int                   objc,
                                          Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_vsrc_loc_get self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
...
SWIGINTERN int _wrap_OpenIRA_help(ClientData clientData SWIGUNUSED,
                                  Tcl_Interp*           interp,
                                  int                   objc,
                                  Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp, objc, objv, "o:OpenIRA_help self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
...
SWIGINTERN int _wrap_OpenIRA_import_lef(ClientData clientData SWIGUNUSED,
                                        Tcl_Interp*           interp,
                                        int                   objc,
                                        Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_lef self lef ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_import_def(ClientData clientData SWIGUNUSED,
                                        Tcl_Interp*           interp,
                                        int                   objc,
                                        Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_def self def ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_import_sdc(ClientData clientData SWIGUNUSED,
                                        Tcl_Interp*           interp,
                                        int                   objc,
                                        Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_sdc self sdc ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_set_top_module(ClientData clientData SWIGUNUSED,
                                            Tcl_Interp*           interp,
                                            int                   objc,
                                            Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_set_top_module self verilogModule ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_import_verilog(ClientData clientData SWIGUNUSED,
                                            Tcl_Interp*           interp,
                                            int                   objc,
                                            Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_verilog self verilog ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_import_lib(ClientData clientData SWIGUNUSED,
                                        Tcl_Interp*           interp,
                                        int                   objc,
                                        Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_lib self lib ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_import_db(ClientData clientData SWIGUNUSED,
                                       Tcl_Interp*           interp,
                                       int                   objc,
                                       Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_import_db self dbLoc ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_read_voltage_src(ClientData clientData SWIGUNUSED,
                                              Tcl_Interp*           interp,
                                              int                   objc,
                                              Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(interp,
                   objc,
                   objv,
                   "oo:OpenIRA_read_voltage_src self vsrc ",
                   (void*) 0,
                   (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
...
SWIGINTERN int _wrap_OpenIRA_analyze_power_grid(ClientData  clientData
                                                            SWIGUNUSED,
                                                Tcl_Interp* interp,
                                                int         objc,
                                                Tcl_Obj* CONST objv[])
{
  ...
  if (SWIG_GetArgs(
          interp, objc, objv, "o:OpenIRA_analyze_power_grid self ", (void*) 0)
      == TCL_ERROR)
    SWIG_fail;
  res1 = SWIG_ConvertPtr(objv[1], &argp1, SWIGTYPE_p_OpenIRA, 0 | 0);
  if (!SWIG_IsOK(res1)) {
...
static swig_attribute swig_OpenIRA_attributes[]
    = {{"-db", _wrap_OpenIRA_db_get, _wrap_OpenIRA_db_set},
       {"-verilog_stor",
        _wrap_OpenIRA_verilog_stor_get,
        _wrap_OpenIRA_verilog_stor_set},
       ...
       {"-top_cell_name",
        _wrap_OpenIRA_top_cell_name_get,
        _wrap_OpenIRA_top_cell_name_set},
...
static swig_class  _wrap_class_OpenIRA       = {"OpenIRA",
                                         &SWIGTYPE_p_OpenIRA,
                                         _wrap_new_OpenIRA,
                                         swig_delete_OpenIRA,
                                         swig_OpenIRA_methods,
                                         swig_OpenIRA_attributes,
                                         swig_OpenIRA_bases,
                                         swig_OpenIRA_base_names,
                                         &swig_module,
                                         SWIG_TCL_HASHTABLE_INIT};

static swig_command_info swig_commands[] = {
    {SWIG_prefix "new_OpenIRA", (swig_wrapper_func) _wrap_new_OpenIRA, NULL},
    {SWIG_prefix "delete_OpenIRA",
     (swig_wrapper_func) _wrap_delete_OpenIRA,
     NULL},
    {SWIG_prefix "OpenIRA_db_set",
     (swig_wrapper_func) _wrap_OpenIRA_db_set,
     NULL},
    {SWIG_prefix "OpenIRA_db_get",
     (swig_wrapper_func) _wrap_OpenIRA_db_get,
     NULL},
    {SWIG_prefix "OpenIRA_verilog_stor_set",
     (swig_wrapper_func) _wrap_OpenIRA_verilog_stor_set,
     NULL},
    {SWIG_prefix "OpenIRA_verilog_stor_get",
     (swig_wrapper_func) _wrap_OpenIRA_verilog_stor_get,
     NULL},
    {SWIG_prefix "OpenIRA_lib_stor_set",
     (swig_wrapper_func) _wrap_OpenIRA_lib_stor_set,
     NULL},
    {SWIG_prefix "OpenIRA_lib_stor_get",
     (swig_wrapper_func) _wrap_OpenIRA_lib_stor_get,
     NULL},
    {SWIG_prefix "OpenIRA_sdc_file_set",
     (swig_wrapper_func) _wrap_OpenIRA_sdc_file_set,
     NULL},
    {SWIG_prefix "OpenIRA_sdc_file_get",
     (swig_wrapper_func) _wrap_OpenIRA_sdc_file_get,
     NULL},
    {SWIG_prefix "OpenIRA_top_cell_name_set",
     (swig_wrapper_func) _wrap_OpenIRA_top_cell_name_set,
     NULL},
    {SWIG_prefix "OpenIRA_top_cell_name_get",
     (swig_wrapper_func) _wrap_OpenIRA_top_cell_name_get,
     NULL},
    {SWIG_prefix "OpenIRA_vsrc_loc_set",
     (swig_wrapper_func) _wrap_OpenIRA_vsrc_loc_set,
     NULL},
    {SWIG_prefix "OpenIRA_vsrc_loc_get",
     (swig_wrapper_func) _wrap_OpenIRA_vsrc_loc_get,
     NULL},
    {SWIG_prefix "OpenIRA_help", (swig_wrapper_func) _wrap_OpenIRA_help, NULL},
    {SWIG_prefix "OpenIRA_import_lef",
     (swig_wrapper_func) _wrap_OpenIRA_import_lef,
     NULL},
    {SWIG_prefix "OpenIRA_import_def",
     (swig_wrapper_func) _wrap_OpenIRA_import_def,
     NULL},
    {SWIG_prefix "OpenIRA_import_sdc",
     (swig_wrapper_func) _wrap_OpenIRA_import_sdc,
     NULL},
    {SWIG_prefix "OpenIRA_set_top_module",
     (swig_wrapper_func) _wrap_OpenIRA_set_top_module,
     NULL},
    {SWIG_prefix "OpenIRA_import_verilog",
     (swig_wrapper_func) _wrap_OpenIRA_import_verilog,
     NULL},
    {SWIG_prefix "OpenIRA_import_lib",
     (swig_wrapper_func) _wrap_OpenIRA_import_lib,
     NULL},
    {SWIG_prefix "OpenIRA_import_db",
     (swig_wrapper_func) _wrap_OpenIRA_import_db,
     NULL},
    {SWIG_prefix "OpenIRA_read_voltage_src",
     (swig_wrapper_func) _wrap_OpenIRA_read_voltage_src,
     NULL},
    {SWIG_prefix "OpenIRA_analyze_power_grid",
     (swig_wrapper_func) _wrap_OpenIRA_analyze_power_grid,
     NULL},
    {SWIG_prefix "OpenIRA",
     (swig_wrapper_func) SWIG_ObjectConstructor,
     (ClientData) &_wrap_class_OpenIRA},
    {0, 0, 0}};

static swig_var_info swig_variables[] = {{0, 0, 0, 0}};

static swig_const_info swig_constants[] = {{0, 0, 0, 0, 0, 0}};

/* -------- TYPE CONVERSION AND EQUIVALENCE RULES (BEGIN) -------- */

static swig_type_info _swigt__p_OpenIRA
    = {"_p_OpenIRA", "OpenIRA *", 0, 0, (void*) &_wrap_class_OpenIRA, 0};
static swig_type_info _swigt__p_char
    = {"_p_char", "char *", 0, 0, (void*) 0, 0};
static swig_type_info _swigt__p_odb__dbDatabase
    = {"_p_odb__dbDatabase", "odb::dbDatabase *", 0, 0, (void*) 0, 0};
...
static swig_type_info _swigt__std__ptrdiff_t
    = {"_std__ptrdiff_t", "std::ptrdiff_t", 0, 0, (void*) 0, 0};
static swig_type_info _swigt__std__size_t
    = {"_std__size_t", "std::size_t", 0, 0, (void*) 0, 0};
...
static swig_cast_info _swigc__p_OpenIRA[]
    = {{&_swigt__p_OpenIRA, 0, 0, 0}, {0, 0, 0, 0}};
static swig_cast_info _swigc__p_char[]
    = {{&_swigt__p_char, 0, 0, 0}, {0, 0, 0, 0}};
static swig_cast_info _swigc__p_odb__dbDatabase[]
    = {{&_swigt__p_odb__dbDatabase, 0, 0, 0}, {0, 0, 0, 0}};
static swig_cast_info _swigc__p_std__vectorT_std__string_t[]
    = {{&_swigt__p_std__vectorT_std__string_t, 0, 0, 0}, {0, 0, 0, 0}};
static swig_cast_info _swigc__std__ptrdiff_t[]
    = {{&_swigt__std__ptrdiff_t, 0, 0, 0}, {0, 0, 0, 0}};
static swig_cast_info _swigc__std__size_t[]
    = {{&_swigt__std__size_t, 0, 0, 0}, {0, 0, 0, 0}};
  ...
  for (i = 0; i < swig_module.size; ++i) {
    ...
    if (type->clientdata) {
      ...
      for (meth = klass->methods; meth && meth->name; ++meth) {
        int            newEntry;
        Tcl_HashEntry* hashentry
            = Tcl_CreateHashEntry(&(klass->hashtable), meth->name, &newEntry);
...
SWIGEXPORT int SWIG_init(Tcl_Interp* interp)
{
  ...
  for (i = 0; swig_variables[i].name; i++) {
    Tcl_SetVar(
        interp, (char*) swig_variables[i].name, (char*) "", TCL_GLOBAL_ONLY);
    Tcl_TraceVar(interp,
                 (char*) swig_variables[i].name,
                 TCL_TRACE_READS | TCL_GLOBAL_ONLY,
                 (Tcl_VarTraceProc*) swig_variables[i].get,
                 (ClientData) swig_variables[i].addr);
    Tcl_TraceVar(interp,
                 (char*) swig_variables[i].name,
                 TCL_TRACE_WRITES | TCL_GLOBAL_ONLY,
                 (Tcl_VarTraceProc*) swig_variables[i].set,
                 (ClientData) swig_variables[i].addr);
  }
